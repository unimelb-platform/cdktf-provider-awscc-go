...
func (j *jsiiProxy_M2EnvironmentStorageConfigurationsList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*M2EnvironmentStorageConfigurations:
		val := val.(*[]*M2EnvironmentStorageConfigurations)
	...
	case []*M2EnvironmentStorageConfigurations:
		val_ := val.([]*M2EnvironmentStorageConfigurations)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*M2EnvironmentStorageConfigurations; received %#v (a %T)", val, val)
		}
	}
