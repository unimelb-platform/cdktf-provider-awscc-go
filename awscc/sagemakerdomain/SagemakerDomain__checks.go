...
func (s *jsiiProxy_SagemakerDomain) validatePutTagsParameters(value interface{}) error {
	if value == nil {
		return fmt.Errorf("parameter value is required, but nil was provided")
	}
	switch value.(type) {
	case cdktf.IResolvable:
		// ok
	case *[]*SagemakerDomainTags:
		value := value.(*[]*SagemakerDomainTags)
	...
	case []*SagemakerDomainTags:
		value_ := value.([]*SagemakerDomainTags)
		value := &value_
		for idx_cd4240, v := range *value {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter value[%#v]", idx_cd4240) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(value) {
			return fmt.Errorf("parameter value must be one of the allowed types: cdktf.IResolvable, *[]*SagemakerDomainTags; received %#v (a %T)", value, value)
		}
	}
