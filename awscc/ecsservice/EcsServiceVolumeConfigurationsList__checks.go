...
func (j *jsiiProxy_EcsServiceVolumeConfigurationsList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*EcsServiceVolumeConfigurations:
		val := val.(*[]*EcsServiceVolumeConfigurations)
	...
	case []*EcsServiceVolumeConfigurations:
		val_ := val.([]*EcsServiceVolumeConfigurations)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*EcsServiceVolumeConfigurations; received %#v (a %T)", val, val)
		}
	}
