...
func (j *jsiiProxy_SagemakerFeatureGroupThroughputConfigOutputReference) validateSetInternalValueParameters(val interface{}) error {
	switch val.(type) {
	case cdktf.IResolvable:
		// ok
	case *SagemakerFeatureGroupThroughputConfig:
		val := val.(*SagemakerFeatureGroupThroughputConfig)
		if err := _jsii_.ValidateStruct(val, func() string { return "parameter val" }); err != nil {
			return err
		}
	case SagemakerFeatureGroupThroughputConfig:
		val_ := val.(SagemakerFeatureGroupThroughputConfig)
		val := &val_
		if err := _jsii_.ValidateStruct(val, func() string { return "parameter val" }); err != nil {
			return err
		}
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *SagemakerFeatureGroupThroughputConfig; received %#v (a %T)", val, val)
		}
	}
