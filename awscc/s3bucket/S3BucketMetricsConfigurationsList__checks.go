...
func (j *jsiiProxy_S3BucketMetricsConfigurationsList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*S3BucketMetricsConfigurations:
		val := val.(*[]*S3BucketMetricsConfigurations)
	...
	case []*S3BucketMetricsConfigurations:
		val_ := val.([]*S3BucketMetricsConfigurations)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*S3BucketMetricsConfigurations; received %#v (a %T)", val, val)
		}
	}
