...
func (j *jsiiProxy_GuarddutyDetectorDataSourcesOutputReference) validateSetInternalValueParameters(val interface{}) error {
	...
	case cdktf.IResolvable:
		// ok
	case *GuarddutyDetectorDataSources:
		val := val.(*GuarddutyDetectorDataSources)
		if err := _jsii_.ValidateStruct(val, func() string { return "parameter val" }); err != nil {
			return err
		}
	case GuarddutyDetectorDataSources:
		val_ := val.(GuarddutyDetectorDataSources)
		val := &val_
		if err := _jsii_.ValidateStruct(val, func() string { return "parameter val" }); err != nil {
			return err
		}
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *GuarddutyDetectorDataSources; received %#v (a %T)", val, val)
		}
	}
