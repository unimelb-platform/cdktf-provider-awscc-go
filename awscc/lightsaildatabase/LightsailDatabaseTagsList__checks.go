...
func (l *jsiiProxy_LightsailDatabaseTagsList) validateAllWithMapKeyParameters(mapKeyAttributeName *string) error {
...
func (j *jsiiProxy_LightsailDatabaseTagsList) validateSetInternalValueParameters(val interface{}) error {
	...
	case []*LightsailDatabaseTags:
		val_ := val.([]*LightsailDatabaseTags)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*LightsailDatabaseTags; received %#v (a %T)", val, val)
		}
	}
