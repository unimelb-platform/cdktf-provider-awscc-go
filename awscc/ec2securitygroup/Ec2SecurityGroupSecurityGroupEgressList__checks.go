...
func (j *jsiiProxy_Ec2SecurityGroupSecurityGroupEgressList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*Ec2SecurityGroupSecurityGroupEgress:
		val := val.(*[]*Ec2SecurityGroupSecurityGroupEgress)
	...
	case []*Ec2SecurityGroupSecurityGroupEgress:
		val_ := val.([]*Ec2SecurityGroupSecurityGroupEgress)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*Ec2SecurityGroupSecurityGroupEgress; received %#v (a %T)", val, val)
		}
	}
