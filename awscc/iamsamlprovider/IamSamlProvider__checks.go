...
func (i *jsiiProxy_IamSamlProvider) validatePutTagsParameters(value interface{}) error {
	if value == nil {
		return fmt.Errorf("parameter value is required, but nil was provided")
	}
	switch value.(type) {
	...
	case *[]*IamSamlProviderTags:
		value := value.(*[]*IamSamlProviderTags)
	...
	case []*IamSamlProviderTags:
		value_ := value.([]*IamSamlProviderTags)
		value := &value_
		for idx_cd4240, v := range *value {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter value[%#v]", idx_cd4240) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(value) {
			return fmt.Errorf("parameter value must be one of the allowed types: cdktf.IResolvable, *[]*IamSamlProviderTags; received %#v (a %T)", value, value)
		}
	}
